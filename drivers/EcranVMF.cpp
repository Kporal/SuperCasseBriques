...
void EcranVMF::effacerEcranV(Couleur arPlan) {
	...
	for (int i = 0; i < taille_x; i++) {
		...
		setColonne(i);
		if (i == 0)
			afficherCaractere(BLANC, NOIR, 213);
		if (i == taille_x - 1)
			afficherCaractere(BLANC, NOIR, 184);
		else
			afficherCaractere(BLANC, NOIR, 205);
		setColonne(i);
		setLigne(taille_y - 1);
		if (i == 0)
			afficherCaractere(BLANC, NOIR, 212);
		if (i == taille_x - 1)
			afficherCaractere(BLANC, NOIR, 190);
		else
			afficherCaractere(BLANC, NOIR, 205);
		setLigne(1);
		setColonne(i);
		if (i == 0)
			afficherCaractere(BLANC, NOIR, 198);
		if (i == taille_x - 1)
			afficherCaractere(BLANC, NOIR, 181);
		else
			afficherCaractere(BLANC, NOIR, 205);
	}
	setLigne(0);
...
void EcranVMF::afficherCaractere(Couleur prPlan, Couleur arPlan,
...
void EcranVMF::afficherCaractere(int l, int c, Couleur prPlan, Couleur arPlan,
		const char caractere) {
...
void EcranVMF::deplacement(int x, int y) {
	...
	Un = (x < origineCarre_x) && (x > (origineCarre_x - taille_x))
			&& (y > (origineCarre_y));
	Deux = (origineCarre_y > y) && (y > (origineCarre_y - taille_y))
			&& (x > (origineCarre_x));
	Trois = (x > (origineCarre_x)) && (y > (origineCarre_y));
	Quatre = (x > origineCarre_x - taille_x)
			&& (y < (origineCarre_y - taille_y));
	Cinq = (y > (origineCarre_y - taille_y))
			&& (x < (origineCarre_x - taille_x));
	Six = ((y < (origineCarre_y - taille_y))
			&& (x < (origineCarre_x - taille_x)));
