...
void MemoirePaginee::testP() {
	...
	maPagination.paging_map(ppage_new, vpage_tmp, 0,
	...
	/* Copy the contents of the current page of code to this new page
	 mapping */
	memcpy((void*) vpage_tmp, (void*) vpage_code,
	PAGE_SIZE);

	/* Transfer the mapping of the current page of code to this new page */
	maPagination.paging_map(ppage_new, vpage_code, 0,
	VM_MAP_ATOMIC | VM_MAP_PROT_READ | VM_MAP_PROT_WRITE);
...
void MemoirePaginee::test() {
	MemoirePaginee::testP();
	monEcran->afficherMot(2, 10, "ca marche ", BLANC);
}
...
vaddr_t MemoirePaginee::malloc(size_t nbytes) {
	...
	maPagination.paging_map(ppage, free_adress, false,
			VM_MAP_ATOMIC | VM_MAP_PROT_READ | VM_MAP_PROT_WRITE);
