...
sextant_ret_t Pagination::paging_setup(paddr_t identity_mapping_base,
...
//	maMemoirePhysique=memoire;
	...
	for (paddr = identity_mapping_base; paddr < identity_mapping_top; paddr +=
			PAGE_SIZE) {
		if (paging_setup_map_helper(pd, paddr, paddr))
			return -SEXTANT_ERROR;
	}
	/* Identity-map the PC-specific BIOS/Video area */
	for (paddr = BIOS_N_VIDEO_START; paddr < BIOS_N_VIDEO_END; paddr +=
			PAGE_SIZE) {
		if (paging_setup_map_helper(pd, paddr, paddr))
			return -SEXTANT_ERROR;
	}
...
sextant_ret_t Pagination::paging_map(paddr_t ppage_paddr, vaddr_t vpage_vaddr,
		bool is_user_page, ui32_t flags) {
	...
	struct x86_pde* pd = (struct x86_pde*) (PAGING_MIRROR_VADDR
			+ PAGE_SIZE * virt_to_pd_index(PAGING_MIRROR_VADDR));
	/* Address of the PT in the mirroring */
	struct x86_pte* pt = (struct x86_pte*) (PAGING_MIRROR_VADDR
			+ PAGE_SIZE * index_in_pd);

	/* The mapping of anywhere in the PD mirroring is FORBIDDEN ;) */
	if ((vpage_vaddr >= PAGING_MIRROR_VADDR)
			&& (vpage_vaddr < PAGING_MIRROR_VADDR + PAGING_MIRROR_SIZE))
		return -SEXTANT_ERROR;
...
sextant_ret_t Pagination::paging_unmap(vaddr_t vpage_vaddr) {
	...
	if (maMemoirePhysique.free(pt[index_in_pt].paddr << 12) == SEXTANT_ERROR)
		return -SEXTANT_ERROR;
