...
static ui32_t find_cluster_num(luoffset_t addr) {
	return (((ui32_t) addr - data_struct.FirstDataSector)
			/ data_struct.SecPerClus + 2);
}
 ...
 */
static sextant_ret_t set_cluster_value(vaddr_t buf, int taille,
		ui32_t prec_clus = NULL) {
	...
	for (int i = 0; i < nb_cluster; i++) {
		...
		for (int secteur = 0, lg = data_struct.BytsPerSec, offset = 0;
				secteur < data_struct.SecPerClus && taille > 0; secteur++) {
			ide_write_device(&ide_controllers[0].devices[1], buf + offset,
					addr_clus + secteur, lg);
			offset += lg;
			taille -= lg;
		}
	}
 ...
 */
static sextant_ret_t rewrite_cluster(vaddr_t buf, int taille,
		luoffset_t addr_block) {
	int nb_cluster = taille / data_struct.BytsPerClus + 1;
...
static sextant_ret_t add_fils(luoffset_t addr_p, luoffset_t moi) {
	...
	ide_read_device(&ide_controllers[0].devices[1], (vaddr_t) &p, addr_p,
	...
	if (i == MAX_FILS) //Trop de fils : un répertoire peut contenir au maximum 20 fichiers.
		...
		p.fils[i] = moi;
		rewrite_cluster((vaddr_t) &p, sizeof(struct fat_directory),
				p.addr_block);
		return SEXTANT_OK;
	}
}
...
static luoffset_t get_file(char* name,/*OUT*/struct fat_directory* file) {
	...
	ide_read_device(&ide_controllers[0].devices[1], (vaddr_t) &root, 32,
			sizeof(struct fat_directory));
	for (i = 0; i < MAX_FILS && !compare && root.fils[i] != 0; i++) {
		ide_read_device(&ide_controllers[0].devices[1], (vaddr_t) file,
				root.fils[i], sizeof(struct fat_directory));
...
sextant_ret_t create_file(char* name) {
	...
	struct fat_directory* dir = (struct fat_directory*) malloc(
			sizeof(struct fat_directory));
...
static sextant_ret_t fat_mount(bool* formate) {
	...
	data_struct.fat_table = (ui32_t*) malloc(
			sizeof(ui32_t) * data_struct.CountOfClusters); //Table de la liste des clusters
	data_struct.FATSz32 = sizeof(ui32_t) * data_struct.CountOfClusters
			+ sizeof(struct fat_data_structure) + data_struct.BytsPerSec; //Premier secteur est reserve (detection de la Fat)
	...
	for (int i = 1;
			(i * data_struct.BytsPerClus)
					< (data_struct.NumFATs * data_struct.FATSz32); i++) {
		data_struct.CountOfClusters = data_struct.TotSec32
				/ data_struct.SecPerClus - i;
	...
	struct stock_tab* tintin = (struct stock_tab*) malloc(
			sizeof(struct stock_tab));
	...
	struct fat_directory* root = (struct fat_directory*) malloc(
			sizeof(struct fat_directory));
	...
	*formate = true;	//Une table FAT est présente sur le disque.
	...
	int secteur = 0;
	ide_write_device(&ide_controllers[0].devices[1], (vaddr_t) formate, secteur,
			data_struct.BytsPerSec);
	secteur++;
	//Le secteur 1 contient la structure FAT permettant de retrouver les fichiers écrits sur le disque
	ide_write_device(&ide_controllers[0].devices[1], (vaddr_t) &data_struct,
			secteur, data_struct.BytsPerSec);
	secteur++;
	...
	while (offset < taille) {
		...
		if (SEXTANT_OK
				!= ide_write_device(&ide_controllers[0].devices[1],
						(vaddr_t) tintin, secteur, data_struct.BytsPerSec))
			return -SEXTANT_ERROR;
...
struct fat_directory* fopen(char* nom_fic) {
	struct fat_directory* file = (struct fat_directory*) malloc(
			sizeof(struct fat_directory));
...
void fwrite(struct fat_directory* file, char* chaine, int taille) {
	...
	while (buf[i] != EOF) {	//Lecture du fichier jusqu'à la fin. Ajout en fin de fichier !
		for (nb_sec = 0; nb_sec < data_struct.SecPerClus && buf[i] != EOF;
				nb_sec++) {
			ide_read_device(&ide_controllers[0].devices[1], (vaddr_t) buf,
					addr_data + nb_sec, data_struct.BytsPerSec);
	...
	if (libre_sec > 0) {	//On remplit le dernier secteur lu
		for (j = 0; j < libre_sec && taille > 0; j++, i++, taille--)
			buf[i] = a_ecrire[j];
		ide_write_device(&ide_controllers[0].devices[1], (vaddr_t) buf,
				addr_data + nb_sec, data_struct.BytsPerSec);
		nb_sec++;
	}
	//On remplit le cluster
	for (; nb_sec < data_struct.SecPerClus && taille > 0; nb_sec++) {
		ide_write_device(&ide_controllers[0].devices[1], (vaddr_t) a_ecrire + j,
				addr_data + nb_sec, data_struct.BytsPerSec);
	...
	if (taille > 0) {//Allocation d'un nouveau cluster, beaucoup de choses à écrire
		set_cluster_value((vaddr_t) a_ecrire + j, taille,
				find_cluster_num(addr_data));
	}
}
...
void fread(struct fat_directory* file,/*OUT*/char* lu, int taille) {
	...
	while (buf[ind] != EOF && taille > 0) {
		for (int sec = 0;
				sec < data_struct.SecPerClus && buf[ind] != EOF && taille > 0;
				sec++) {	//Recherche EOF dans tous les secteurs du cluster
			ide_read_device(&ide_controllers[0].devices[1], (vaddr_t) buf,
					addr_data + add_sector + sec, data_struct.BytsPerSec);
			for (ind = 0; buf[ind] != EOF && ind < data_struct.BytsPerSec;
					ind++)
				;
			taille -= ind;
...
void fat_setup(bool clear) {
	...
	struct stock_tab* milou = (struct stock_tab*) malloc(
			sizeof(struct stock_tab));	//Permet la récupération de la table
	int secteur = 0;

	if (clear)	//Réinitialisation du système de fichier
		fat_clear();	//Supprime la structure Fat en place
	ide_read_device(&ide_controllers[0].devices[1], (vaddr_t) formate, secteur,
			512);
	secteur++;
	if (!*formate)	//Pas de table FAT, on en crée une
		fat_mount(formate);
	else {	//On recupere les infos de la FAT
		ide_read_device(&ide_controllers[0].devices[1], (vaddr_t) &data_struct,
				secteur, sizeof(struct fat_data_structure));
		secteur++;
		...
		for (ui32_t offset = 0; offset < taille; offset += 512, secteur++) {
			ide_read_device(&ide_controllers[0].devices[1], (vaddr_t) milou,
					secteur, 512);
			for (int i = 0;
					i < 128 && data_struct.CountOfClusters > (offset / 4 + i);
					i++) {
