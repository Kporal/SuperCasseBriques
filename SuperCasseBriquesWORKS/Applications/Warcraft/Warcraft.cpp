...
Warcraft::Warcraft(EcranV *ecran, ClavierV *clavierV, Souris *mouse) :
		ecr(ecran), clavier(clavierV), jeux(false), horloge(0), mymouse(mouse) {
}
...
void Warcraft::actualiserLeMenu() {
	...
	menu->changerValeur(Menu::POPULATION,
			joueurHumain->nombreGuerriers() + joueurHumain->nombrePaysanBois()
					+ joueurHumain->nombrePaysanOr());
	menu->changerValeur(Menu::PAYSAN,
			joueurHumain->nombrePaysanBois() + joueurHumain->nombrePaysanOr());
