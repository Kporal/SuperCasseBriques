...
int MapTD::gererLesEnnemi() {
	...
	for (int i = 0; i < tailleChemin; i++) {
		if (contientEnnemi(i))
			ecr->afficherCaractere(che[i]->x, che[i]->y, JAUNE, MARRON, ENNEMI);
		else
			ecr->afficherCaractere(che[i]->x, che[i]->y, MARRON, MARRON,
					CHEMIN);
	}
	return vainqueur;
}
...
int MapTD::gererLesTours() {
	...
	for (int i = 0; i < nombreTours; i++) {
		if (tours[i]->round()) {
			if (tours[i]->caractere != 'z'
					&& attaquer(tours[i], positionsTours[i]))
				k++;
			else
				k += attaquerZone(tours[i], positionsTours[i]);
		}
	}
	return k * 5;
}
