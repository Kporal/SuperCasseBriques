...
MissileAlien::MissileAlien(TimerSpaceInvaders *timer, int degats, int n,
		int ligne, int colonne, char *a, int h, int l, Couleur c) :
...
MissileAlien *MissileAlien::lancerNouveauMissile(int positionLigne,
		int positionColonne) {
	if (missileDisponible()) {
		...
		missile = new MissileAlien(timer, 1, numero, positionLigne,
				positionColonne, "|", 1, 1, ROUGE);
