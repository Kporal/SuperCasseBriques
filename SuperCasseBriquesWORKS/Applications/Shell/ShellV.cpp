...
#include <Applications/SuperCasseBriques/CasseBriques.h>
...
void ShellV::runAppli(String cmd, int numThread) {
	...
	if(!arg_1.isEqual(0)&&!arg_1.isEqual(vide)) {
		...
		if(arg_1.isEqual("CB")) {
							sem.P();
							ecran->miniprintf("run lance Super Casse Brique de ouf de la mort qui tue !!!\n");
							CasseBriques *SCB = new CasseBriques(ecran,clavier);
							SCB->start();
							block.P(); // on bloque le shell

							sem.V();
					}
		if(arg_1.isEqual("Pong")) {
					sem.P();
					//sched->addThread((thread*) getAppliAlancer("Compteur"), numThread);
					ecran->miniprintf("run lance SuperPong\n");

					SuperPong *SPong = new SuperPong(ecran,clavier);
					SPong->start();
					block.P(); // on bloque le shell
