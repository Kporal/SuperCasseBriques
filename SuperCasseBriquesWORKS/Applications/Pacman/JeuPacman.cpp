...
void JeuPacman::run() {
	...
	ecran->afficherMot(24, LARGEUR / 2 - 21,
			"Appuyez sur q pour quitter le jeu", BLANC);
...
void JeuPacman::launch() {
	...
	while (lePlateau->jouer) {
		...
		case BAS:
			...
			break;
		case DROITE:
			if (!lePlateau->tab[lePlateau->getPacman()->getX()][lePlateau->getPacman()->getY()
					+ 1].estMur()) {
				lePlateau->getPacman()->setDirectionNext(NULLE);
...
int JeuPacman::initLevel() {
	...
	while (!lePlateau->touche) {
		...
		ecran->afficherMot(5, 20, "Appuyez sur une touche pour commencer",
				BLANC);
