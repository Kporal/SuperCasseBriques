	...
	while (count < notMarked) {
		...
		for (int dir = 0; dir <= 3; dir++) {
			...
			if ((!mark[x][y]) && (adjMatrix[xclose][yclose][dir] > 0)) {
				...
				if (distance[x][y]
						> distance[xclose][yclose]
								+ adjMatrix[xclose][yclose][dir]) {

					distance[x][y] = distance[xclose][yclose]
							+ adjMatrix[xclose][yclose][dir];
...
void Dijkstra::printPath(int x, int y) {
	...
	if (x == sourceX && y == sourceY) {
		...
		printPath((int) (predecessor[x][y] / 1000),
				(int) (predecessor[x][y] % 1000));
...
int Dijkstra::nextPosition(int x, int y) {
	...
	if ((int) (predecessor[x][y] / 1000) == sourceX
			&& predecessor[x][y] % 1000 == sourceY) {
		return x * 1000 + y;
	} else
		return nextPosition((int) (predecessor[x][y] / 1000),
				predecessor[x][y] % 1000);
