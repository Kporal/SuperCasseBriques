...
void TimerSpaceInvaders::afficherChiffreCommente(int l, int c,
	...
	for (int i = c + nombreDeCaracteres;
			i < c + nombreDeCaracteres + nbChiffres; i++)
		ecran->afficherCaractere(l, i, NOIR, NOIR, ' ');
	ecran->afficherChiffre(l,
			c + nombreDeCaracteres + nbChiffres - tailleChiffre, valeur);
}
...
void TimerSpaceInvaders::handler(Espace *espace) {
	...
	while (!modifTic) {
		...
		if (compteurTicHandler != past) {
			modifTic = true;
			past = compteurTicHandler;
		} else
			thread_yield();
	}
	...
	if ((compteurTicHandler
			- (previoustimerDeplacementAliens + (timerDeplacementAliens)))
			> 0) {
		previoustimerDeplacementAliens = previoustimerDeplacementAliens
				+ (timerDeplacementAliens);
		deplacementAliens->V();
	}

	if ((compteurTicHandler
			- (previoustimerMissileAliens + (timerMissileAliens))) > 0) {
		previoustimerMissileAliens = previoustimerMissileAliens
				+ (timerMissileAliens);
		for (int i = 0; i < MISSILES_ALIENS_MAX; i++)
			missileAliens[i]->V();
	}
	...
	if ((compteurTicHandler
			- (previoustimerMissilesCanon + (timerMissilesCanon))) > 0) {
		previoustimerMissilesCanon = previoustimerMissilesCanon
				+ timerMissilesCanon;
		missileCanon->V();
	}
	...
	afficherChiffreCommente(0, 34, "Lives remaining:",
			espace->getCanon()->getPointsDeVie(), 1);
	afficherChiffreCommente(0, 55, "Level:", espace->getNiveau(), 1);

	if (false) {
		int posLigne = 2;
		int posColonne = 60;
		afficherChiffreCommente(posLigne++, posColonne, "Timer (ms):",
				compteurTicHandler, 5);
		posLigne++;
		afficherChiffreCommente(posLigne++, posColonne, "Sem. Alien:",
				deplacementAliens->Valeur(), 5);
		afficherChiffreCommente(posLigne++, posColonne, "Nbre MissC:",
				MissileCanon::getNombre(), 5);
		afficherChiffreCommente(posLigne++, posColonne, "Nbre MissA:",
				MissileAlien::getNombre(), 5);
		posLigne++;
//		ecran->afficherChiffreCommente(posLigne++, posColonne, "Nbr Threads:", getNbThreads(), 2);
		afficherChiffreCommente(posLigne++, posColonne, "NbrThrReady:",
				number_thread_ready, 2);
		afficherChiffreCommente(posLigne++, posColonne, "NbThrWaiting:",
				number_thread_waiting, 2);
		posLigne++;
		BlocAliens *bloc;
		bloc = espace->getBlocAliens();
		afficherChiffreCommente(posLigne++, posColonne, "PositBlocLigne:",
				bloc->getPositionLigne(), 2);
		afficherChiffreCommente(posLigne++, posColonne, "PosBlocColonne:",
				bloc->getPositionColonne(), 2);
		afficherChiffreCommente(posLigne++, posColonne, "Direction Bloc:",
				bloc->getDirection(), 2);
		posLigne++;
		afficherChiffreCommente(posLigne++, posColonne, "LargeurBloc:",
				bloc->getLargeur(), 2);
		afficherChiffreCommente(posLigne++, posColonne, "ecartGauche:",
				bloc->getEcartGauche(), 2);
		afficherChiffreCommente(posLigne++, posColonne, "HauteurBloc:",
				bloc->getHauteur(), 2);
		afficherChiffreCommente(posLigne++, posColonne, "NombreAlien:",
				bloc->getNombreAliensRestants(), 2);
	}
