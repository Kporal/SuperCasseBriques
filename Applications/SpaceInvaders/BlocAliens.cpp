...
BlocAliens::BlocAliens(TimerSpaceInvaders *t, Espace *e, const int niveau,
		int posL, int posC) {
	timer = t;
	espace = e;
	...
	if (niveau % 2 == 0) {
		...
		for (int i = 0; i < largeur; i++) {
			...
			if (hauteur >= 6)
				ajouterAlien(ligne++, i,
						EntiteFactory::creerAlienRobot(niveau));
...
void BlocAliens::retirerAlien(Alien *alien) {
	...
	blocAliens[getPositionLigne(alien) * LARGEUR_BLOC_ALIEN_MAX
			+ getPositionColonne(alien)] = NULL;
