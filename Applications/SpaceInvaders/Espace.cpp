...
Espace::Espace(EcranV *ec, TimerSpaceInvaders *t, ClavierV *c, int n) {
	...
	MissileAlien::setNombreMaxMissiles(
			Config::getNombreMissilesAliensMax(niveau));

	timer->setTimerDeplacementAliens(
			Config::getVitesseDeplacementBlocAlien(niveau));
	...
	for (int numBunk = 0; numBunk < nbBunker; numBunk++) {
		for (int ligneBunk = 0; ligneBunk < hauteurBunker; ligneBunk++) {
			for (int colBunk = 0; colBunk < largeurBunker; colBunk++) {
				if (!(ligneBunk == 0
						&& (colBunk == 0 || colBunk == largeurBunker - 1)
						|| ligneBunk == hauteurBunker - 1
								&& colBunk == largeurBunker / 2))
					ajouterEntite(
							EntiteFactory::creerBunker(bunkerY + ligneBunk,
									bunkerX + colBunk + 80 / nbBunker * numBunk,
									couleur, resistance));
			}
		}
	}
