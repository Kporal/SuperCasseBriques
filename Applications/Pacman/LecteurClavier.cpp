...
LecteurClavier::LecteurClavier(/*EcranV *e,*/ClavierV *c,
		PlateauPacman *plateau) {
	//ecran=e;
	clavier = c;
	lePlateau = plateau;
}
...
void LecteurClavier::run() {
	...
	while (lePlateau->jouer) {
		...
		if (src = clavier->getChar()) {
			...
			//haut
			case 'z':
				lePlateau->getPacman()->setDirectionNext(HAUT);
				p->ecrireMot("HAUT\n");
				...
				//gauche
			case 'q':
				lePlateau->getPacman()->setDirectionNext(GAUCHE);
				break;

				//bas
			case 's':
				lePlateau->getPacman()->setDirectionNext(BAS);
				break;

				//droite
			case 'd':
				lePlateau->getPacman()->setDirectionNext(DROITE);
				break;
			}
		}
	}
